...
import com.example.dialectexample.config.datasource.QueryDslConfig;
...
public abstract class RepositoryTest {
    ...
    private static final DockerImageName MARIA_DB_VERSION = DockerImageName.parse("mariadb:10.3.36");

    // static 으로 사용해야 1번만 초기화 (도커 올리고 내리는 과정이 시간이 걸리므로 1번만 하는 것이 좋을듯)
    @Container
    protected static final MariaDBContainer<?> MARIA_DB = new MariaDBContainer<>(MARIA_DB_VERSION) // 지정하지 않으면 테이블 이름은 test
